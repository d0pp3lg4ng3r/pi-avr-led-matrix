...
update:                                     ;callback for overflow of TCNT0
    ld      YH,         high(bytebuff)      ;load the high byte of the address into the pointer
    ld      YL,         low(bytebuff)       ;load the low byte of the address into the pointer
...
loop:                                       ;begin pwm byte processing loop 
    ...
    cp      TCNT1H,     BYTE_BUF            ;compare the current byte value and the Timer1 High byte value
...
i2c_ack:
    ...
    cp      TEMP,       PINA                ;check if the SCL pin was set
